...
class PointTransformerV3(PointModule):
    ...
    def forward(self, data_dict, latent=False):
        """
        ...
        enc_point = point.copy()
        if not self.cls_mode:
            point = self.dec(point)
        if latent == True:
            return enc_point, point
        return point
...
def load_ptv3():
    ...
    ckpt = torch.load("./pretrained/ptv3-pretrained.pth")

            
    model.load_state_dict(ckpt)
    return model

def load_miniptv3(enc_pcl_dim):
    model = PointTransformerV3(
        in_channels=3,
        order=("z", "z-trans", "hilbert", "hilbert-trans"),
        stride=(2, 2),
        enc_depths=(1, 1, 1),
        enc_channels=(32, 32, enc_pcl_dim),
        enc_num_head=(2, 4, 4),
        enc_patch_size=(120, 120, 120),
        # dec_depths=(2, 2, 2, 2),
...
#         stride=(2, 2, 2),
#         enc_depths=(1, 1, 1, 1),
#         enc_channels=(32, 64, 128, enc_pcl_dim),
#         enc_num_head=(2, 4, 8, 8),
#         enc_patch_size=(120, 120, 120, 120),
#         # dec_depths=(2, 2, 2, 2),
#         # dec_channels=(64, 64, 128, 128),
#         # dec_num_head=(4, 4, 8, 16),
#         # dec_patch_size=(120, 120, 120, 120),
...
if __name__=="__main__":
    ...
    query_points = torch.stack((x, y, z), dim=-1).float().unsqueeze(0).cuda()
    query_points = query_points.view(1, 40*40*40, 3)[:,:100,:].repeat(bs,1,1)

    x = torch.rand((bs, n_points, 3)).reshape(-1,3).cuda()
    batch = torch.arange(0,bs).unsqueeze(1).repeat(1, n_points).reshape(-1).long().cuda()
    # model = PointTransformerV3(
    #     in_channels=6,
    #     order=("z", "z-trans", "hilbert", "hilbert-trans"),
    #     stride=(2, 2, 2, 2),
    ...
    #     pdnorm_adaptive=False,
    #     pdnorm_affine=True,
    #     pdnorm_conditions=("ScanNet", "S3DIS", "Structured3D"),).cuda()
    
    # ckpt = torch.load("/home/data/pretrained/ptv3-pretrained.pth",map_location=torch.device('cuda:0'))

            
    # model.load_state_dict(ckpt)
    # torch.save(model.state_dict(), 'ptv3-pretrained.pth')
    
    model = load_miniptv3(128).cuda()

    data_dict = {
        'feat': torch.rand(bs*n_points,3).cuda(),
        'coord': x,
        'grid_size': 0.02,
        'batch': batch,
    }
