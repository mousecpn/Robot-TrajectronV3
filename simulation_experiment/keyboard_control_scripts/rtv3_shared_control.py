...
def init_ros2_visualization(checkpoint_path=None):
    """Initialize ROS2 visualization nodes in separate thread"""
    global ros_viz_node
    global trajectron_node
    
    if checkpoint_path is None:
        checkpoint_path = os.path.join(os.path.dirname(__file__), '../..', 'checkpoints', 'line24.pth')
    ...
    def ros_spin():
        ...
        trajectron_node = TrajectronNode(checkpoint_path=checkpoint_path)
...
def init_model(config_path=None, checkpoint_path=None):
    ...
    if checkpoint_path is None:
        checkpoint_path = os.path.join(os.path.dirname(__file__), '../..', 'checkpoints', 'line9.pth')
