...
def write_scene_grasps(root, scene_id, grasp_matrix):
    path = root / "scene_grasps" / (scene_id + ".npz")
    # assert not path.exists()
    np.savez_compressed(path, grasps=grasp_matrix)
    return scene_id

def read_scene_grasps(root, scene_id):
    path = root / "scene_grasps" / (scene_id + ".npz")
    return np.load(path)["grasps"]

def read_mesh(root, scene_id, name="mesh_pose_list"):
    path = root / name / (scene_id + ".npz")
    return np.load(path, allow_pickle=True)['pc']

def write_mesh(root, scene_id, point_cloud, name="mesh_pose_list"):
    path = root / name / (scene_id + ".npz")
    np.savez_compressed(path, pc=np.array(point_cloud, dtype=object))
