...
import numpy as np
import IPython
from . import AssistancePolicyOneGoal as GoalPolicy
...
class AssistancePolicy:
  ...
  def get_assisted_action(self, goal_distribution, ig_action_twist=None, fix_magnitude_user_command=True):
    """
    Get assisted action \n
    Args: 
      goal_distribution
    ...
    # combine with ig_action with dynamic weighting based on entropy
    if ig_action_twist is not None:
      # compute entropy of goal distribution
      entropy = -np.sum(goal_distribution * np.log(goal_distribution+1e-8))
      # print("entropy: " + str(entropy))
      # weight more on ig_action when entropy is high
      w_1 = entropy if entropy > 0.1 else 0.1
      w_2 = 1 / w_1
      # scale ig_action_twist to have same magnitude as total_action_twist
      pos_norm = np.linalg.norm(total_action_twist[:3])
      rot_norm = np.linalg.norm(total_action_twist[3:])
      ig_action_twist_scaled = np.zeros(action_dimension)
      if np.linalg.norm(ig_action_twist[:3]) > 0:
        ig_action_twist_scaled[:3] = ig_action_twist[:3] / np.linalg.norm(ig_action_twist[:3]) * pos_norm
      if np.linalg.norm(ig_action_twist[3:]) > 0:
        ig_action_twist_scaled[3:] = ig_action_twist[3:] / np.linalg.norm(ig_action_twist[3:]) * rot_norm
      total_action_twist = w_1 / (w_1 + w_2) * ig_action_twist_scaled + w_2 / (w_1 + w_2) * total_action_twist
      # print("after ig_action: " + str(total_action_twist))
