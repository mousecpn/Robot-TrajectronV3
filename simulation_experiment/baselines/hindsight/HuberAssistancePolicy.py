import numpy as np
import IPython
from . import AssistancePolicyOneTarget
...
class HuberAssistancePolicy(AssistancePolicyOneTarget.AssistancePolicyOneTarget):
  ...
  def update(self, robot_state, user_action, robot_state_after_action):
    ...
    self.quat_curr = R.from_matrix(robot_state.ee_trans[:3,:3]).as_quat()
    self.quat_after_trans = R.from_matrix(self.robot_state_after_action.ee_trans[:3,:3]).as_quat()
  ...
  def get_qderivative_rotation(self):
    goal_quat = get_closer_symmetry_goal_quat(self.goal_quat, self.quat_after_trans)
    goal_quat = self.goal_quat
    quat_between = (R.from_quat(goal_quat) * R.from_quat(self.quat_after_trans).inv()).as_quat()
    ...
    if (np.sum(goal_quat * self.quat_after_trans)) > 0:
...
#      print 'rotdir good'
  ...
  ROTATION_MULTIPLIER = 0.125 # (0.07) TODO: tune this parameter
...
def transition_quaternion(quat, angular_vel, action_apply_time):
  ...
  return R.from_rotvec(action_apply_time*angular_vel).as_quat() * quat